...
import { stakeholderData, getStakeholderData } from '../data/mockData'
      ...
      <div className="flex-1 overflow-y-auto pb-1">
        ...
        <div className="px-4 space-y-3">
          ...
          <div className="flex gap-3">
            <MetricCard
              label="Total Farms"
              value={regionData.totalFarms}
              icon={<Map size={16} color="#1976D2"/>}
              bgColor="#E3F2FD"
            />
            <MetricCard
              label="Total Alerts"
              value={regionData.totalAlerts}
          ...
          <div className="card p-4">
            ...
            <p className="text-4xl font-bold text-primary mb-4">{regionData.avgHealthScore}%</p>
            <ResponsiveContainer width="100%" height={90}>
              <LineChart data={regionData.healthTrend}>
            ...
            <div className="flex items-center gap-4">
              <PieChart width={120} height={120}>
                <Pie
                  ...
                  dataKey="value"
                >
                  {regionData.alertsByType.map((entry, i) => (
                    <Cell key={i} fill={entry.color}/>
                  ))}
                </Pie>
              </PieChart>
              <div className="flex-1 space-y-2">
                {regionData.alertsByType.map((item, i) => (
          ...
          <div className="card p-4 mb-4">
            ...
            <div className="space-y-2">
              ...
              {stakeholderData.farmsData.map((row, i) => (
                <button
                  key={i}
                  className="w-full flex gap-2 items-center p-2.5 rounded-xl transition-colors"
                  style={{ background: selectedRegion === row.region ? '#E8F5E9' : '#F8F9F4' }}
